...
public partial class Main : Form
{
    ...
    private static bool HandleError(NiTEWrapper.NiTE.Status status)
    {
        if (status == NiTEWrapper.NiTE.Status.Ok) return true;
        ...
        try
        {
            if (skel.State == Skeleton.SkeletonState.Tracked)
            {
                ...
                if (joint1.Position.Z > 0 && joint2.Position.Z > 0)
                {
                    ...
                    g.DrawEllipse(new Pen(Brushes.White, 5), new Rectangle(joint1PosEllipse, new System.Drawing.Size(5, 5)));
                    g.DrawEllipse(new Pen(Brushes.White, 5), new Rectangle(joint2PosEllipse, new System.Drawing.Size(5, 5)));
                }
            }
        }
        catch (Exception)
    ...
    {
        if (HandleError(NiTEWrapper.NiTE.Initialize()))
            Text = @"Running by NiTE v" + NiTEWrapper.NiTE.Version;
        else
            Environment.Exit(1);
    }
        ...
        using (var frame = userTracker.ReadFrame())
        {
            ...
            lock (this)
            {
                ...
                using (var g = Graphics.FromImage(image))
                {
                    ...
                    {
                        if (user.IsNew && user.IsVisible) userTracker.StartSkeletonTracking(user.UserId);
